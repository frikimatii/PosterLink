...
async function auth(req, res, next) {
    ...
    if (!header) return res.status(401).json({ error: "Acceso no autorizado. Se requiere token." });
...
app.get("/", (req, res) => res.json({ message: "API de PosterLink funcionando correctamente" }));
...
app.post("/login", authLimiter, async (req, res) => {
    try {
        ...
        res.json({ token, user: { userId: user._id, name: user.name, email: user.email, isPremium: user.isPremium } });
    } catch (err) {
...
app.post("/get-video-info", auth, async (req, res) => {
    ...
    try {
        ...
        const title = match ? match[1].replace(" - YouTube", "").trim() : "Título no disponible";
        ...
        res.status(500).json({ error: "No se pudo procesar el video. Inténtalo de nuevo." });
    }
});
...
app.post("/upload-to-imgbb", auth, async (req, res) => {
    ...
    if (!imageData || !filename) {
        return res.status(400).json({ error: "Datos de imagen y nombre de archivo son requeridos." });
    }
    if (!IMGBB_API_KEY) {
        return res.status(500).json({ error: "Clave de API de imgbb no configurada en el servidor." });
    }
...
app.post("/update-to-premium", auth, async (req, res) => {
    try {
        ...
        res.json({
            ...
            user: { userId: req.user._id, name: req.user.name, email: req.user.email, isPremium: req.user.isPremium }
        });
    } catch (error) {
